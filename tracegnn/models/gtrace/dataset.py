...
def init_config(config: ExpConfig):
    ...
    config.DatasetParams.operation_cnt = 100
    config.DatasetParams.service_cnt = 100
    config.DatasetParams.status_cnt = 100

    # Set runtime info
    if config.RuntimeInfo.latency_range is None:
        tmp_latency_dict: Dict[int,List[float]] = {}

        # TODO: ERROR IndexError: index 30 is out of bounds for dimension 0 with size 2
        ...
        # config.RuntimeInfo.latency_range = torch.zeros([100, 2], dtype=torch.float)
        # config.RuntimeInfo.latency_p98 = torch.zeros([100], dtype=torch.float)
        ...
        for op, vals in tmp_latency_dict.items():
            ...
            # if op < config.DatasetParams.operation_cnt:
            #     config.RuntimeInfo.latency_range[op] = torch.tensor([vals_mean, vals_std])
            #     config.RuntimeInfo.latency_p98[op] = np.percentile(vals, 98)
            # else:
            #     logger.warning(f"Operation index {op} is out of bounds.")
