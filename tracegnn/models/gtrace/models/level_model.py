...
class TreeLSTMLevelEncoder(nn.Module):
    def __init__(self, config: ExpConfig):
        ...
        # TODO: modify treeLSTM to graphLSTM
        # DGL GCN Encoder
        if config.Model.latency_input:
            if config.Model.optimize_model == 'tree_lstm':
                ...
                self.tree_lstm = GraphLSTM(config.Model.embedding_size * 3 + config.Latency.latency_feature_length,
                                          config.Model.graph_embedding_size * 2)
        else:
            if config.Model.optimize_model == 'tree_lstm':
                self.tree_lstm = TreeLSTM(config.Model.embedding_size * 3, config.Model.graph_embedding_size * 2)
            else:
                self.tree_lstm = GraphLSTM(config.Model.embedding_size * 3, config.Model.graph_embedding_size * 2)
...
class LevelDecoder(nn.Module):
    def __init__(self, config: ExpConfig):
        ...
        # self.graph_lstm = GraphLSTM(config.Model.graph_embedding_size,config.decoder_max_nodes * config.Model.decoder_feature_size * 2)
    ...
    def forward(self, graph_encode: torch.Tensor):
        ...
        # TODO: graphLSTM
        # decoded = self.graph_lstm(graph_encode).reshape(
        #     graph_encode.size(0), graph_encode.size(1), self.config.decoder_max_nodes, -1)
...
class TreeLSTMLatencyEncoder(nn.Module):
    def __init__(self, config: ExpConfig):
        ...
        if config.Model.latency_input:
            ...
            #     )
            # else:
            #     self.linear_input = GraphLSTM(config.Model.embedding_size * 3, config.Model.latency_feature_size * 2)

            self.linear_input = TreeLSTM(config.Model.embedding_size * 3, config.Model.latency_feature_size * 2)

    def forward(self, g: dgl.DGLGraph, embed: torch.Tensor):
        y = self.linear_input(g, embed.unsqueeze(1))
        # y = self.linear_input(embed.unsqueeze(1))
...
class TreeLSTMLatencyDecoder(nn.Module):
    def __init__(self, config: ExpConfig):
        ...
        # else:
        #     self.tree_lstm = GraphLSTM(config.Model.latency_feature_size, config.Model.latency_feature_size)
...
class LevelModel(nn.Module):
    ...
    def forward(self, g: dgl.DGLGraph, n_z: int=None):
        ...
        # if self.config.Model.latency_model == 'gat' or self.config.Model.optimize_model == 'graph_lstm':
        #     z_latency_mu = z_latency_mu.expand([-1, n_z, -1])
        #     z_latency_logvar = z_latency_logvar.expand([-1, n_z, -1])
        # else:
        #     z_latency_mu = z_latency_mu.unsqueeze(1).expand([-1, n_z, -1])
        #     z_latency_logvar = z_latency_logvar.unsqueeze(1).expand([-1, n_z, -1])

        if self.config.Model.optimize_model == 'graph_lstm' and self.config.Model.latency_model != 'gat':
            z_latency_mu = z_latency_mu.expand([-1, n_z, -1])
            z_latency_logvar = z_latency_logvar.expand([-1, n_z, -1])
        else:
            z_latency_mu = z_latency_mu.unsqueeze(1).expand([-1, n_z, -1])
